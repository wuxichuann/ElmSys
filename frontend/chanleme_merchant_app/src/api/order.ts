...
/**
 * @function getNewOrdersApi
 * @description 获取商家的新订单列表 (status: 'placed')。
 * @returns {Promise<Order[]>} 新订单数组。
 */
export const getNewOrdersApi = async (): Promise<Order[]> => {
  const response = await apiClient.get('/orders/restaurant/new');
  return response.data;
};

/**
 * @function getInProgressOrdersApi
 * @description 获取商家的进行中订单列表。
 * @returns {Promise<Order[]>} 进行中订单数组。
 */
export const getInProgressOrdersApi = async (): Promise<Order[]> => {
  const response = await apiClient.get('/orders/restaurant/in-progress');
  return response.data;
};

/**
 * @function getHistoryOrdersApi
 * @description 获取商家的历史订单列表。
 * @returns {Promise<Order[]>} 历史订单数组。
 */
export const getHistoryOrdersApi = async (): Promise<Order[]> => {
  const response = await apiClient.get('/orders/restaurant/history');
  return response.data;
};
 ...
 */
export const confirmOrderApi = async (orderId: number): Promise<Order> => {
  const response = await apiClient.patch(`/orders/${orderId}/confirm`);
  return response.data.order; // 后端返回 { message, order }
};

/**
 * @function updateOrderStatusApi
 * @description 商家更新订单状态（如 'preparing', 'ready_for_pickup'）。
 * @param {number} orderId - 订单ID。
 * @param {UpdateOrderStatusDto} data - 包含新状态的 DTO。
 * @returns {Promise<Order>} 更新后的订单对象。
 */
export const updateOrderStatusApi = async (orderId: number, data: UpdateOrderStatusDto): Promise<Order> => {
